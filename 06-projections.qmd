...
The `Mercator` CRS is not well-defined at the poles $\varphi = \pm 90^\circ$. It is common to restrict
the visualization to the $-80^\circ \le \varphi \le 84^\circ$ latitude range.
...
```{julia}
viz(grid |> Proj(LambertCylindrical), showsegments = true)
```

```{julia}
viz(grid |> Proj(GallPeters), showsegments = true)
```
...
```{julia}
viz(grid |> Proj(Robinson), showsegments = true)
```

```{julia}
viz(grid |> Proj(WinkelTripel), showsegments = true)
```
