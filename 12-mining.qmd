...
`Kriging`, `Interpolate`, `InterpolateNeighbors`, `Shadow`, `Map`, `Filter`,
`variogram`, `boundingbox`, `convexhull`, `viewer`
...
Let's fit a theoretical variogram model to each (independent) variable:
...
gs = [variogram(samples, i, maxlag = maxlag) for i in 1:nv]

γs = [GeoStatsFunctions.fit(ExponentialVariogram, g) for g in gs]
```

We can plot the fitted model on top of the empirical variogram as follows:

```{julia}
function gammaplot(g, γ)
  fig = funplot(g, maxlag = maxlag)
...
```{julia}
interps = map(1:nv) do i
  samples |> Select(i) |> InterpolateNeighbors(blocks, model=Kriging(γs[i]))
end
...
Given the grade of copper and metallurgical recovery for each block, we can proceed
...
The expression `[estim recov]` concatenates the columns of the
two geotables assuming that they share the same geospatial domain.
It is equivalent to `hcat(estim, recov)`.
...
```{julia}
model |> Filter(x -> x.value > 0) |> Select("value") |> viewer
```
...
```{julia}
model |> Filter(x -> x.value > 0 && x.S < 0.25) |> Select("value") |> viewer
```
