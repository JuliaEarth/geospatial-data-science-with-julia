...
The explanation of this transform is beyond the scope of this book. Nevertheless, it is a simple
mathematical expression in terms of logarithms of ratios (e.g., Cu/S).

Next, we `Detrend` the samples to facilitate the inference of geospatial correlation models, and
attempt to transform the multivariate distribution to a multivariate standard normal using the
`ProjectionPursuit` transform:

```{julia}
grades |> CLR() |> Detrend() |> ProjectionPursuit() |> values |> pairplot
```
...
In order to "undo" all these transforms after the interpolation, we will create a revertible
pipeline:

```{julia}
preproc = CLR() → Detrend() → ProjectionPursuit()
...
Let's inspect the multivariate distribution to see if it resembles the original distribution
in the drill hole samples:

```{julia}
#estim |> Sample(10000) |> values |> pairplot
```
