...
Now that we did set the right expectations, let's address an important question
that many readers might have coming from other communities:

> **Do we gain anything by not adhering to existing interfaces?**
...
Notice how the `isconvex` function is compiled away to the constant `true` when called on
the triangle. The code for a generic polygon is much more evolved and requires runtime
checks that are too expensive to afford, specially in 3D.

Having cleared that up, we will now proceed to the final chapter of the first part of the
book, which covers the advanced geometric processing features of the framework.