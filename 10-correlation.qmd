...
After empirical estimation with geospatial data, the next step consists of
fitting a theoretical model. This is done in order to be able to evaluate
the variogram function at any lag $h$, not just the specific lags of the
empirical variogram.

We can fit a specific theoretical model:

```{julia}
fit(SphericalVariogram, g)
```

Or let the framework select the model with minimum weighted least-squares error:
