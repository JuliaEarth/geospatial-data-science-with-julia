...
If we plot the values of the variable `Z` in the horizontal axis and
the values of the same variable measured at lag $h=0$ on the vertical
axis, we get points along the identity line (i.e. no scatter):

```{julia}
hscatter(img, lag=0.0)
```
...
```{julia}
hscatter(img, lag=3.0)
```

```{julia}
hscatter(img, lag=5.0)
```

```{julia}
hscatter(img, lag=10.0)
```

```{julia}
hscatter(img, lag=50.0)
```
...
We can use the `variogram` to estimate the function at specific
lag values:

```{julia}
g = variogram(img, maxlag = 50.0)
```
...
The `dir` option can be used to estimate the function along specific directions:

```{julia}
gₕ = variogram(img, dir=(1.0, 0.0), maxlag = 50.0)
gᵥ = variogram(img, dir=(0.0, 1.0), maxlag = 50.0)
...
```{julia}
gₚ = variogramsurface(img, maxlag = 50.0)
```
...
- The selection of an appropriate theoretical variogram model for interpolation of
  ...
  `variogram`, and then we `fit` a theoretical model. The most widely
