...
```{julia}
tmesh1 = refine(mesh, TriRefinement())
tmesh2 = refine(tmesh1, TriRefinement())

fig = Mke.Figure()
viz(fig[1, 1], tmesh1, showsegments = true)
viz(fig[1, 2], tmesh2, showsegments = true)
fig
```

```{julia}
qmesh1 = refine(mesh, QuadRefinement())
qmesh2 = refine(qmesh1, QuadRefinement())

fig = Mke.Figure()
viz(fig[1, 1], qmesh1, showsegments = true)
viz(fig[1, 2], qmesh2, showsegments = true)
fig
```
...
- The geotable representation generalizes traditional GIS representations
  ("raster" vs. "vector"), and enables a unified approach to visualization
