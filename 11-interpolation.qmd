...
samples = img |> Sample(10000, replace=false, rng=Xoshiro(123))

samples |> viewer
```

Our goal is to interpolate the variable `Z` over the original domain.
Let's start by estimating the `variogram` from the samples.
...
```{julia}
g = variogram(samples, maxlag = 100.0)
```
...
```{julia}
g = variogram(samples, maxlag = 100.0, estimator = :cressie)
```
...
- Models such as `Kriging` exploit **geospatial correlation** to improve
  ...
  1. Estimate the `variogram` from the available samples
