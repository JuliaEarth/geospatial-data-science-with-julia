...
The `Interpolate` transform uses neighborhoods over th geospatial domain of the geotable
for high-performance interpolation with big data. An alternative approach is provided by
the `InterpolateGlobal` transform, wich uses all available data to "fit" the model before
...
```{julia}
interp = samples |> Interpolate(img.geometry, "Z" => Kriging(γ))
```
