...
as geotables, let's produce our first visualizations of the data.
...
a `viz`/`viz!` function for efficient visualization of geotables.

For illustration purposes, let's create a small geotable over a
`CartesianGrid`:
...
We can visualize two or more geotables in the same scene
...
The `viz`/`viz!` function is very powerful. It can be used to visualize geospatial
variables over multiple geospatial domains with a consistent set of aesthetic options.
Nevertheless, it is still not enough for geospatial data science as explained in the
next section.
...
A variable is "viewable" if its values can be converted into colors with
the `ascolors` function.
...
You are now equiped with a set of visualization functions that can
really improve the speed at which you explore and analyze geospatial data.
