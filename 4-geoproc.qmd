...
a n-dimensional `Box` with the corner points or a n-dimensional `Ball` with
...
On the other hand, a geometry is a `Polytope` if it can be represented
as a combination of "flat sides", which are also `Polytope` themselves.
A 3D `Polytope` is called a `Polyhedron`, a 2D `Polytope` is called a
`Polygon` and a 1D `Polytope` is called a polygonal `Chain`. All these
geometries are represented internally with a list of vertices.
