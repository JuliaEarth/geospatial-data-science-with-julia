...
is related to the traditional [join](https://en.wikipedia.org/wiki/Join_(SQL)) of
two tables.
...
It is always a good idea to provide units to all `values` in a
geotable. The GeoStats.jl framework is integrated with the
[Unitful.jl](https://github.com/PainterQubits/Unitful.jl) project.
To add the unit "meter" to the numeric value `1`, we can write `1u"m"`.
Similarly, we can add the unit to a vector of values:
...
By default, the `left` kind is used. Like in traditional join, the `geojoin` is not
commutative. If we swap the order of the geotables, the result will be completely different:
...
To learn more about the different `kind`s of database-style join, check the
[DataFrames.jl](https://dataframes.juliadata.org/stable/man/joins) documentation.
...
- There are various `kind`s of `geojoin` such as `inner` and `left`, which match the behavior
  of traditional join in databases. We recommend the DataFrames.j documentation to learn more.
