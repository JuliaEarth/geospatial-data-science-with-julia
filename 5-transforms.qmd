...
divided into `Ordered` and `Multiclass`. All scientific types are currently documented
at [ScientificTypes.jl](https://github.com/JuliaAI/ScientificTypes.jl).

To convert (or coerce) the scientific types of columns in a geotable, we can use the
`Coerce` transform:

```{julia}
gt |> Coerce(:b => Multiclass)
```
