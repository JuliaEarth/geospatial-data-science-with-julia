...
In GIS, another very important coordinate transform is the `Reproject` transform.
...
In our framework, the `Reproject` transform is just another coordinate transform.
...
```{julia}
Ab = letters |> Potrace("color")
```

We can visualize the resulting geometries as usual:

```{julia}
viz(Ab.geometry, color = ["teal", "slategray3"])
```
...
In the next chapters, we will review map projections with the `Reproject`
