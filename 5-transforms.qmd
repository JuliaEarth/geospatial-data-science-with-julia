...
The `Coalesce` transform can be used to replace `missing` values by a specific value:

```{julia}
georef((a=[1,missing,3], b=[4,5,6])) |> Coalesce(value=2)
```
