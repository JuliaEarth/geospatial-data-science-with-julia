...
The number of geometries (elements) of a domain is
...
We will give more examples of "vector data" in the chapter
...
- Julia provides a rich ecosystem for customizing table representations,
  and the flexibility of the language enabled us to create a new table
  representation that is performant and simple.
- A geotable provides all the data access patterns of a `DataFrame`.
  In particular, we can use the syntax `gdf[rows,cols]` to retrieve
  any subset of the data without creating copies of geometries.

It is very convenient to manipulate a geotable as if it was a `DataFrame`.
Nevertheless, we will learn that this is not enough. Advanced geospatial
data science requires higher-level constructs to preserve geospatial
information, which we will cover in [Part II](#4-feattrans.qmd) and
[Part III](#7-splitcombine.qmd) of the book.