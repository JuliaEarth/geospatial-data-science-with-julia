...
Before we can start our journey in geospatial data science, we need to
define what **geospatial data** is and what it is not. The following
definition is simple and useful:
...
@Quinn2023. The concept is independent of the machine
...
It is easy to create new table representations in Julia if
none of the available representations meet the requirements.
...
The number of geometries (or elements) in the domain is
...
The geometry type can be ommited in which case the function assumes that
...
All the data access patterns provided by a `DataFrame` are supported by a `GeoTable`,
with the main difference being the memory optimizations for high-performance geospatial
data science.

Notice that `GeoTable` representation is general enough to accommodate both "raster data"
...
GeoTables.load("data/geotable.geojson")
```

We will see more examples of "vector data" in the chapter
[Interfacing with GIS](3-geoio.qmd), and will explain why
file formats like
...
You will start to realize the benefits of treating "raster data" and
"vector data" with the same representation in the next chapter with
the introduction of a universal viewer for geospatial data.
...
- A `GeoTable` provides all the data access patterns of a `DataFrame`.
  In particular, it supports the syntax `gdf[rows,cols]` without
  without expensive copies of geometries.
...
these constructs in [Part II](#4-feattrans.qmd) and [Part III](#7-splitcombine.qmd)
of the book.