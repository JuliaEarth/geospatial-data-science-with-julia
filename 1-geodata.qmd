...
In our framework, domains are represented as collections of
geometries that can be connected to each other via a topological
relation. This will become clearer after we see a few examples.

All available domain representations come from the
[Meshes.jl](https://github.com/JuliaGeometry/Meshes.jl)
package. Let's start illustrating the most common
...
The number of geometries (or elements) of a domain is obtained
with the `nelements` function:

```{julia}
nelements(grid)
```

Even though we can operate on this domain as if it was a
"soup of geometries", the major advantage this abstraction
provides is the `topology` of the domain:
