...
As a final example of geospatial transform, let's consider the following geotable stored in a `.png` file:
...
In the opposite direction, we have the `Rasterize` transform, which takes a geotable over a `GeometrySet`
and assigns the geometries to a `Grid`. In this transform, we can provide an external grid for the assignments
or request a grid size to discretize the `boundingbox` of the geometries.

Let's illustrate the transform with the following geotable:
...
There are five polygons in the domain:

```{julia}
gt |> viewer
```

We can discretize these polygons with a grid of size 20x20:
...
The values of the variables are aggregated at geometric intersections
using a default aggregation function, which can be overwritten with a
user option.
