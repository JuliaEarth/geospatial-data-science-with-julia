...
The `÷` operator in Julia performs *interger* division, so `10÷2` yields `5`. Conversely, `10/2` performs *floating point* division and yields a float, `5.0`.
...
```{julia}
st = georef((a=[1, 2, 2, 2, 3, 3], b=[1, 2, 3, 4, 5, 6])) |> Coerce("a" => Multiclass, "b" => Continuous)
```
...
The higher the tolerance, the less the number of segments in the boundary:
